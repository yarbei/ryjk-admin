<template>
    <el-tabs class="el_tab" v-model="activeName" :stretch="true" @tab-click="handleClick" style="padding: 30px; background-color: #fff">
      <!--基本信息-->
          ...
          <el-row :gutter="100">
            <el-col :span="8"  :offset="4">
              <el-form-item label="创建时间">
                <el-date-picker type="date" placeholder="选择日期" v-model="form1.createDate" style="width: 100%;" disabled></el-date-picker>
              </el-form-item>
            </el-col>
            <el-col :span="8">
        ...
        <el-col>
          ...
          <el-card class="box-card grtz_box" v-for="item in sfjyArray" :key="item.sfjlId">
            <div slot="header" class="clearfix grtz_title">
              <span>{{item.visitDate}}<span class="f-right">{{item.status ==1?"已随访":"未随访"}}</span></span>
            </div>
            <div class="text item grtz_content">
              <p><span>随访建议 : {{item.visitRecord.content}}</span></p>
            </div>
          </el-card>
      ...
      <el-tab-pane label="个人体征" name="grtz">
        ...
        <el-card class="box-card grtz_box" v-for="item in grtzArray" :key="item.bodySignTypeId">
          <div slot="header" class="clearfix grtz_title">
            <span>{{item.bodySignType}}<span class="grtz_titleTime" v-if="!(item.createTime == null)">({{item.createTime}})</span></span>
            <p><span>初始{{item.bodySignType}}记录 : {{item.oldValue.value}}</span>  <span>最新{{item.bodySignType}}记录 : {{item.value}}</span> <span class="f-right">{{item.sureUpdate == 0?"未更新":"已更新"}}</span></p>
          </div>
          <div class="text item grtz_content" :id="'grtz_chart'+item.bodySignTypeId">
