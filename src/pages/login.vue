...
<div class="fullscreen">
  <div class="login-box">
    ...
    <p class="text-tips">
      <span class="footer-text">{{appName}} &nbsp;<el-tag size="mini">{{version}}</el-tag></span>
...
export default {
  ...
  methods: {
    ...
    userLogin () {
      ...
      that.isLoging = true;
      that.$http.get('/api'+`/user/login?userAccount=${that.username}&password=${that.password}&roleType=${that.roleType}`)
        .then(res=>{
          if (res.data){
            ...
            that.$store.commit('SET_LOGIN_TOKEN', '4eea90fd-2752-481d-ae67-c75f8641a94a');

            that.isLoging = false;

            that.$router.push({name: 'home'});

            console.log(res.data,"8888888888888888888888");
